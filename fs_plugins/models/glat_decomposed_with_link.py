...
class GlatDecomposedLink(FairseqNATModel):
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        if self.args.decode_strategy in ["lookahead", "greedy"]:
            ...
            output_tokens = torch.tensor(output_tokens, device=encoder_out["encoder_out"][0].device, dtype=decoder_out.output_tokens.dtype)
        ...
        return decoder_out._replace(
            ...
            output_scores=torch.full(output_tokens.size(), 1.0).to(output_tokens.device),
            attn=None,
            history=history,
        )
