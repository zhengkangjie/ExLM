...
class LogSumExpAB(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input1, input2):
        with torch.no_grad():
            output = logsumexp_keepdim(input1 + input2, dim=1)
    ...
    def backward(ctx, grad_output):
        ...
        with torch.no_grad():
            inf_mask = (output == -float('inf'))
            output.masked_fill_(inf_mask, 0)
            X = input1 + input2
            X_inf_mask = (X == -float('inf'))
            X.masked_fill_(X_inf_mask, 0)
            res = torch.exp(X - output)
            res.masked_fill_(X_inf_mask, 0)
            if ctx.needs_input_grad[0]:
                grad_input1 = res * grad_output
            if ctx.needs_input_grad[1]:
                grad_input2 = res * grad_output
        return grad_input1, grad_input2

my_logsumexp = LogSumExpAB.apply

# @jit.script
def loop_function_noempty(last_f: Tensor, links: Tensor, match: Tensor) -> Tensor:
    f_next = my_logsumexp(last_f, links) # batch * 1 * prelen
